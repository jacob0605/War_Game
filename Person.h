...
public:
    ...
    int get_ID() { return baseData.id_num; }                                                       // 객체의 ID 번호를 돌려준다.
    ...
    Real_Pair get_delta() const noexcept { return delta; } // 임의로 추가한 함수, 이동중 공격 명령을 받아 멈춘 경우 공격을 멈추면 다시 이동하게 하기 위함.
    int get_health() const noexcept { return health; }
    void set_health(int hp) { health = hp; }
    int get_speed() const noexcept { return health; }
