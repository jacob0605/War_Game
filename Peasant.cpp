...
void Peasant::set_load(double in_load) {
    ...
    */
    if (in_load > capacity) {
        ...
        baseData.speed = max_speed - (load / capacity) * max_speed;
        if (baseData.speed < MIN_SPEED) {
            baseData.speed = MIN_SPEED;
        }
        cout << "With load of " << load << " can move at speed " << baseData.speed << endl;
    }
}
...
void Peasant::display_status() {
    ...
    */
    ...
    if (baseData.health > 0) { // get_alive() == true
        ...
        const Real_Pair destination = get_destination();

        cout << "Peasant ID: " << baseData.id_num << " at (" << location.x << ", " << location.y << ") is alive" << endl;
        cout << "Load is " << load << ", health is " << baseData.health << endl;
        cout << "Moving at speed " << baseData.speed << " to (" << destination.x << ", " << destination.y << ")" << endl;
    } else {
        const Real_Pair location = get_location();
        cout << "Peasant ID: " << baseData.id_num << " at (" << location.x << ", " << location.y << ") is dead" << endl;
    }
